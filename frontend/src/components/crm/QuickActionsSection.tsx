...
function QuickActionCard({
  ...
  return (
    <button
      onClick={onClick}
      disabled={isLoading}
      className="group relative overflow-hidden rounded-[24px] border border-white/10 bg-gradient-to-br from-white/5 to-white/[0.02] p-5 text-left transition-all hover:border-[#35d6ae]/30 hover:bg-[#35d6ae]/5"
    >
      <div className="absolute right-0 top-0 h-32 w-32 translate-x-8 translate-y-[-8px] rounded-full bg-[#35d6ae] opacity-0 blur-3xl transition-all group-hover:opacity-5" />
      <div className="relative z-10 flex items-start gap-3">
        <div className="rounded-xl bg-[#35d6ae]/10 p-2.5 text-[#35d6ae] group-hover:bg-[#35d6ae]/20">
          {icon}
        </div>
        <div className="flex-1">
          <h3 className="font-semibold text-white">{title}</h3>
          <p className="mt-1 text-sm text-[#9fb0c8]">{description}</p>
        </div>
      </div>
    </button>
  );
