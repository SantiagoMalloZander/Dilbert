    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      {data.metrics.map((metric, index) => {
        ...
        return (
          <Card
            key={metric.label}
            className="border-white/10 bg-[#07101b] text-white shadow-[0_22px_60px_rgba(2,6,23,0.22)]"
          >
            <CardHeader>
              <div className="mb-4 flex items-start justify-between gap-4">
                <div className="inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-[#35d6ae]/12 text-[#35d6ae]">
              ...
              <CardDescription className="text-[#9fb0c8]">{metric.description}</CardDescription>
            </CardHeader>
            {metric.benchmark ? (
...
export function LeadsByStageSection({ data }: { data: LeadsByStageMetric[] }) {
  return (
    <Card className="border-white/10 bg-[#07101b] text-white shadow-[0_22px_60px_rgba(2,6,23,0.22)]">
      <CardHeader className="flex-row items-start justify-between gap-4">
        <div>
          <CardTitle className="text-white">Leads por etapa</CardTitle>
          <CardDescription className="text-[#9fb0c8]">
    ...
    <Card className="border-white/10 bg-[#07101b] text-white shadow-[0_22px_60px_rgba(2,6,23,0.22)]">
      ...
      <CardContent className="overflow-x-auto">
        <table className="w-full min-w-[640px] text-sm">
          <thead>
            <tr className="border-b border-white/8 text-left text-[#9fb0c8]">
          ...
          <tbody>
            {data.length === 0 ? (
              <tr>
                <td colSpan={5} className="py-8 text-center text-[#6f85a3]">
                  Todavía no hay actividad registrada.
                </td>
              </tr>
            ) : (
              data.map((activity) => (
                <tr key={activity.id} className="border-b border-white/8">
                  <td className="py-3">
                  ...
                  <td className="py-3 text-[#9fb0c8]">{activity.relatedLabel}</td>
                  <td className="py-3 text-[#9fb0c8]">{activity.userName}</td>
                  <td className="py-3 text-right text-[#9fb0c8]">
  ...
  return (
    <Card className="border-white/10 bg-[#07101b] text-white shadow-[0_22px_60px_rgba(2,6,23,0.22)]">
      ...
      <CardContent className="space-y-3">
        {data.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-white/10 px-4 py-8 text-sm text-[#6f85a3]">
          ...
          data.map((lead) => (
            <div
              key={lead.id}
              className="flex flex-col gap-3 rounded-2xl border border-white/8 bg-white/5 px-4 py-4 md:flex-row md:items-center md:justify-between"
            >
              <div>
      ...
      <CardContent className="overflow-x-auto">
        <table className="w-full min-w-[560px] text-sm">
          ...
          <tbody>
            {data.length === 0 ? (
              <tr>
                <td colSpan={4} className="py-8 text-center text-[#6f85a3]">
                  Todavía no hay vendedores activos con datos.
                </td>
              </tr>
            ) : (
              data.map((seller) => (
                <tr key={seller.userId} className="border-b border-white/8">
...
export function DashboardSectionSkeleton({
  ...
  return (
    <div className={cn("rounded-[28px] border border-white/10 bg-[#07101b] p-6", className)}>
